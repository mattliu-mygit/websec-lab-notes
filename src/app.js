...
class Notes {
  ...
  edit_note(id, author, raw) {
    undefsafe(this.note_list, id + ".author", author); //id needs to be [__proto__.]
...
app
  ...
  .post(function (req, res) {
    ...
    if (id && author && enote && !isNaN(parseInt(id))) {
...
app.route("/img").get(function (req, res) {
  if (req.query.id.includes("../") || req.query.id.includes("..\\"))
    res.status(405).send("Invalid url!");
  else
    res.sendFile(path.join(__dirname, "/images/", req.query.id || "jhu.png"));
});
